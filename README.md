...
Listings show a short responsibility summary, evidence-based qualification checks (match / gap / unknown), benefits mentioned in the source, and expandable original text. German is compared with the profile CEFR level; a German-language advert alone is not proof of a German requirement. Explicit levels are scoped to German rather than borrowed from an adjacent English requirement. Strong-language wording is a potential gap for B1 without inventing an exact CEFR level. Preferred skills remain labelled as preferences. Summaries use local extraction rules, so they are fast and offline-capable but are not exhaustive semantic assessments or translations. The source listing remains available.
