...
http.createServer(async (req, res) => {
  ...
  if (['/api/summary','/api/prepare','/api/browser','/api/setup'].includes(url.pathname)) {
    ...
    return (url.pathname==='/api/summary'?summaryHandler:url.pathname==='/api/setup'?setupHandler:url.pathname==='/api/browser'?browserHandler:prepareHandler)(req,res);
  }
  ...
  const allowed = /^\/(?:index\.html|practice-application\.html|styles\.css|app\.js|service-worker\.js|manifest\.json|lib\/(?:model|insights|summaries)\.js|icons\/[a-z0-9.-]+)$/;
