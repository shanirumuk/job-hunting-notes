...
export function roleInsights(job,profile={}) {
 ...
 return {requirements,duties:sections.duties.map(conciseDuty),benefits:sections.benefits.map(s=>({label:concise(s),source:s})),terms:sections.terms.filter(s=>!school.includes(s)).map(concise),questions,warnings,german,internship,commercial,decision:commercial?'Lower priority':school.length?'Check eligibility':requirements.some(r=>r.status==='gap'&&!r.preferred)?'Requirement gap':'Review the requirements'};
}
