...
export function startDeviceSync({state,applied,notify}){
 ...
 function showPairing(code,label){
  content.innerHTML=`<p>${escape(label)}</p><p>On your phone, open this app → Transfer & sync → paste this code.</p><textarea id="device-share-code" readonly aria-label="Private pairing code">${code}</textarea><button class="primary-button" data-device="copy">Copy pairing code</button><p>Your laptop keeps everything. Waiting for the phone to confirm its saved files…</p>`;
  say('Pairing ready. Keep this code private.');
 }
 async function send(){
  const token=await owner();
  const pending=await getItem('device-sending');
  if(pending?.code&&Date.now()-pending.created<900000){const result=await api('pair-status',pending.id,token);if(!result.confirmed){showPairing(pending.code,pending.label);return;}}
  const pack=await current(),baseline=await fingerprint(pack);
  ...
  await setItem('device-sending',{id,code,label:summary(latest),created:Date.now()});
  showPairing(code,summary(latest));
 }
